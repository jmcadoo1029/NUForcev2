...
export async function buildQuotePdf({ qi, ti, lines, budget, budgetOnly = false, output = 'save' }: QuotePdfInput): Promise<QuotePdfBytes | void> {
  ...
  qi = cleanRec(qi)
  ti = cleanRec(ti)
  lines = lines.map((l) => ({ ...l, label: clean(l.label) as string, desc: typeof l.desc === 'string' ? (clean(l.desc) as string) : l.desc }))
  if (budget && Array.isArray(budget.rows)) {
    budget = { ...budget, rows: budget.rows.map((r) => ({ ...r, desc: typeof r.desc === 'string' ? (clean(r.desc) as string) : r.desc })) }
  }
